...
Sessions allow the server to store information about the client so that a user doesn’t need to log in whenever they make a new request.

1. What does bcrypt do to help us store passwords in a secure manner.

Bcrypt hashes the given password so that if the application is breached, the attacker will not be able to easily derive the password from the hash.

1. What does bcrypt do to slow down attackers?

Bcrypt hashes the input received multiple times to increase the time needed for an attacker to break the hash.

1. What are the three parts of the JSON Web Token?

Three parts of a JSON Web Token: header (algorithm type and token type), payload (contains information such as the subject and initialized time of the token), and verify signature of the token.
